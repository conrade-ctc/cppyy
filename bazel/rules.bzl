...
# C++ in-process and need the host toolchain's libc sysroot headers at runtime,
# which a hermetic remote-execution action lacks -- a consumer running tests on
# RBE sets ["no-remote-exec"] to pin them to the local executor.

# The JIT tests need a C++ toolchain (libstdc++ headers + a working clang) at
# RUN time, because clang-repl compiles in-process. Standalone relies on the
# host toolchain (a dev box has gcc/clang); a consumer with no host toolchain
# (e.g. nebula's hermetic CI) instead, via two paired label_flags on the
# consuming module (@cppinterop / @cppyy):
#   - :jit_cxx_data stages its toolchain headers/clang into the test runfiles
#     (default empty -> host), and
#   - :jit_cxx_interp_args supplies the matching runfiles-relative interpreter
#     args as the CPPINTEROP_JIT_CXX_ARGS make-var (default empty -> nothing
#     appended). The args' -stdlib++-isystem/--gcc-toolchain paths must point at
#     what :jit_cxx_data stages.
# Why a make-var and not a string flag / MODULE.bazel arg: the runfiles-relative
# paths can only be formed in a .bzl that resolves repo locations (repo_rloc),
# but the upstream test macros must stay host-based. The make-var lets the
# consumer compute the value in its own .bzl and inject it into these unchanged
# macros via $(...) expansion. _jit_cxx_env appends it to the test env; each
# repo's own :jit_cxx_interp_args flag is added to its tests' toolchains so the
# $(...) resolves. The expansion is empty for the default flag, so standalone is
# unaffected.
...
def _cppinterop_lib_define():
    # Runfiles path of the @cppinterop solib, via $(rlocationpath) so it resolves
    # whether cppinterop is an external repo (cppinterop+/...) or the main repo
    # under test (_main/...). The \" escaping makes the value a string literal in
    # the source (dispatch sources use it verbatim); plain quotes get stripped by
    # Bazel's copt tokenization.
...
def _cppinterop_test_common(name, srcs, extra_copts, extra_deps, data, env, includes):
    return dict(
        ...
        copts = extra_copts,
        data = data + ["@cppinterop//:data", "@cppinterop//:solib",
                       "@cppinterop//:test_solib",
                       "@cppinterop//:cxx_shim/c++", "@llvm//:bin/clang++",
...
def cppinterop_cc_test(name, srcs, extra_copts = [], extra_deps = [], extra_dynamic_deps = [], data = [], env = {}):
    ...
    # Link ONLY the solib (which carries LLVM) -- no static LLVM via
    # llvm_linkopts here, or the test gets a second copy of LLVM's globals and
    # the in-process clang AST asserts ("wrong lexical context"). Mirrors the
    # CMake unittest, which links just clangCppInterOp.
    # -rdynamic exports the test binary's own symbols (CMake's
    # export_executable_symbols) so the JIT can resolve bodies of templates
    # instantiated in the test TU (e.g. instantiation_in_host<int>).
    native.cc_test(
