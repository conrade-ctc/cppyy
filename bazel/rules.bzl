...
load("@llvm//:defs.bzl", "LLVM_CXX_STD")
load("//:defs.bzl", "BASE_COPTS", "CPPINTEROP_BASE_ENV", "CPPINTEROP_COPTS", "repo_rloc")
...
def cppinterop_cc_test(name, srcs, extra_copts = [], extra_deps = [], extra_dynamic_deps = [], data = [], env = {}):
    ...
    cxx_shim_dir = repo_rloc("@cppinterop") + "/cxx_shim"
    base = _cppinterop_test_common(
        ...
        data = data,
        env = CPPINTEROP_BASE_ENV | {"PATH": cxx_shim_dir + ":/usr/bin:/bin"} | env,
...
def cppinterop_dispatch_cc_test(name, srcs):
    ...
    base = _cppinterop_test_common(
        ...
        env = CPPINTEROP_BASE_ENV | {
            "PATH": cxx_shim_dir + ":/usr/bin:/bin",
            "LD_LIBRARY_PATH": CPPINTEROP_BASE_ENV["LD_LIBRARY_PATH"] + ":lib",
        },
        includes = ["include", "unittests/CppInterOp"],
    )
    native.cc_test(**base)
...
def cppyy_py_test(name, sokeys = [], extra_env = {}):
    ...
    native.py_test(
        ...
        env = CPPINTEROP_BASE_ENV | {
            ...
            "LD_LIBRARY_PATH": CPPINTEROP_BASE_ENV["LD_LIBRARY_PATH"] + ":test",
            "CPLUS_INCLUDE_PATH": CPPINTEROP_BASE_ENV["CPLUS_INCLUDE_PATH"] + ":test",
        } | extra_env,
    )
